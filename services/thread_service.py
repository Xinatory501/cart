...
class ThreadService:
    _TICKET_ALPHABET = string.digits
    _TICKET_LENGTH = 6
    ...
    def _user_ticket_key(self, user_id: int) -> str:
        return f"support_ticket_number:{self.profile.key}:{user_id}"

    def _ticket_user_key(self, ticket_number: str) -> str:
        return f"support_ticket_user:{self.profile.key}:{ticket_number}"
    ...
    @staticmethod
    def _normalize_ticket_number(value: str) -> Optional[str]:
        normalized = "".join(ch for ch in (value or "").upper() if ch.isalnum())
        if len(normalized) != ThreadService._TICKET_LENGTH:
            return None
        return normalized

    @classmethod
    def _generate_ticket_number(cls) -> str:
        return "".join(secrets.choice(cls._TICKET_ALPHABET) for _ in range(cls._TICKET_LENGTH))

    def _format_topic_name(self, ticket_number: str) -> str:
        name = f"{self.profile.topic_flag} {ticket_number}".replace("\n", " ").strip()
        if len(name) > 120:
    ...
    @staticmethod
    def _group_link_part(group_id: int) -> str:
        return str(abs(group_id)).replace("100", "", 1)

    def build_thread_link(self, thread_id: int) -> Optional[str]:
        if not self.support_group_id or not thread_id:
            return None
        return f"https://t.me/c/{self._group_link_part(self.support_group_id)}/{thread_id}"
    ...
    async def get_or_create_ticket_number(self, user_id: int) -> str:
        async with get_session() as session:
            config_repo = ConfigRepository(session)
            existing = await config_repo.get(self._user_ticket_key(user_id))
            normalized = self._normalize_ticket_number(existing or "")
            if normalized:
                reverse_key = self._ticket_user_key(normalized)
                reverse_value = await config_repo.get(reverse_key)
                if reverse_value != str(user_id):
                    await config_repo.set(reverse_key, str(user_id))
                
                from database.repository import ChatRepository
                chat_repo = ChatRepository(session)
                active_session = await chat_repo.get_active_session(user_id)
                if active_session and not active_session.ticket_number:
                    active_session.ticket_number = normalized
                    await session.commit()
                    
                return normalized

            ticket_number = self._generate_ticket_number()
            while await config_repo.get(self._ticket_user_key(ticket_number)):
                ticket_number = self._generate_ticket_number()
            await config_repo.set(
                self._user_ticket_key(user_id),
                ticket_number,
                "User support ticket number",
            )
            await config_repo.set(
                self._ticket_user_key(ticket_number),
                str(user_id),
                "Maps ticket number to user id",
            )

            # Update active ChatSession with the new ticket number
            from database.repository import ChatRepository
            chat_repo = ChatRepository(session)
            active_session = await chat_repo.get_active_session(user_id)
            if active_session:
                active_session.ticket_number = ticket_number

            await session.commit()
            return ticket_number

    async def issue_new_ticket_number(self, user_id: int) -> str:
        async with get_session() as session:
            config_repo = ConfigRepository(session)

            previous_value = await config_repo.get(self._user_ticket_key(user_id))
            previous_number = self._normalize_ticket_number(previous_value or "")
            ...
            if previous_number:
                await config_repo.set(
                    self._ticket_user_key(previous_number),
                    str(user_id),
                    "Maps archived ticket number to user id",
                )

            await session.commit()
            return ticket_number

    async def get_ticket_number(self, user_id: int) -> Optional[str]:
        async with get_session() as session:
            config_repo = ConfigRepository(session)
            value = await config_repo.get(self._user_ticket_key(user_id))
            return self._normalize_ticket_number(value or "")

    async def get_user_id_by_ticket_number(self, ticket_number: str) -> Optional[int]:
        normalized = self._normalize_ticket_number(ticket_number)
        if not normalized:
            return None

        async with get_session() as session:
            config_repo = ConfigRepository(session)
            user_id_value = await config_repo.get(self._ticket_user_key(normalized))
            return self._to_int(user_id_value)

    async def get_user_ticket_links(self, user_id: int) -> list[tuple[str, str, int]]:
        async with get_session() as session:
            result = await session.execute(select(Config).where(Config.key.like("support_thread:%")))
        ...
        for row in rows:
            ...
            _, bot_key, stored_user_id = parts
            if self._to_int(stored_user_id) != user_id:
                continue

            thread_id = self._to_int(row.value)
            if not thread_id:
                continue

            link = self.build_thread_link(thread_id)
            if not link:
                continue

            links.append((bot_key, link, thread_id))
    ...
    async def create_thread_for_user(
        ...
        try:
            ...
            ticket_number = (
                await self.issue_new_ticket_number(user_id)
                if force_new_ticket
                else await self.get_or_create_ticket_number(user_id)
            )
            thread_name = self._format_topic_name(ticket_number)
            ...
            info_message = (
                "<b>Новое обращение создано</b>\n\n"
                f"Номер обращения: <code>{ticket_number}</code>\n"
